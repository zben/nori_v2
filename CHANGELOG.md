...
* Change: `NoriV2#find` now ignores namespace prefixes in Hash keys it is searching through.

* Fix: Limited Nokogiri to < 1.6, because v1.6 dropped support for Ruby 1.8.

# 2.2.0 (2013-04-25)

* Feature: [#42](https://github.com/savonrb/nori_v2/pull/42) adds the `:delete_namespace_attributes`
  option to remove namespace attributes like `xmlns:*` or `xsi:*`.

# 2.1.0 (2013-04-21)

* Feature: Added `NoriV2.hash_key` and `NoriV2#find` to work with Hash keys generated by NoriV2.
  Original issue: [savonrb/savon#393](https://github.com/savonrb/savon/pull/393)

# 2.0.4 (2013-02-26)

* Fix: [#37](https://github.com/savonrb/nori_v2/issues/37) special characters
...
* Change: NoriV2 now defaults to use the Nokogiri parser.

* Refactoring: Changed the `NoriV2` module to a class. This might cause problems if you
  included the `NoriV2` module somewhere in your application. This use case was removed
  for overall simplicity.

* Refactoring: Changed the interface to remove any global state. The global configuration
  is gone and replaced with simple options to be passed to `NoriV2.new`.

    ``` ruby
    parser = NoriV2.new(strip_namespaces: true)
    parser.parse(xml)
    ```

* Refactoring: Removed the `NoriV2::Parser` module methods. After refactoring the rest,
  there was only a single method left for this module and that was moved to `NoriV2`.

* Fix: [#16](https://github.com/savonrb/nori_v2/issues/16) strip XML passed to NoriV2.
...
* Fix: Merged [pull request 21](https://github.com/savonrb/nori_v2/pull/21) to fix an
...
* Fix: Merged [pull request 17](https://github.com/savonrb/nori_v2/pull/17) for improved
  xs:time/xs:date/xs:dateTime regular expression matchers.

## 1.1.0 (2012-02-17)

* Improvement: Merged [pull request 9](https://github.com/savonrb/nori_v2/pull/9) to
  allow multiple configurations of NoriV2.

* Fix: Merged [pull request 10](https://github.com/savonrb/nori_v2/pull/10) to handle
...
* Notice: As of v1.0.0, NoriV2 will follow [Semantic Versioning](http://semver.org).
...
* Feature: Added somewhat advanced typecasting:
  ...
  * Strings matching an xs:time, xs:date and xs:dateTime are converted
  ...
  You can disable this feature via:
      ...
      NoriV2.advanced_typecasting = false
